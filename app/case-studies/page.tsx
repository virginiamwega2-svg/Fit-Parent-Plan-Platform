    ...
    <div className="pb-8 sm:pb-10">
      <Reveal>
        <SectionHeader
          ...
          titleLight="Parent case studies"
          titleBold="with real schedules."
        ...
        {parentCaseStudies.map((caseStudy, index) => (
          <Reveal key={caseStudy.id} delayMs={index * 60}>
            <Card className="hover-lift h-full">
              <h2 className="text-lg font-semibold text-foreground">{caseStudy.name}</h2>
              <p className="mt-1 text-sm text-(--color-brand-strong)">{caseStudy.timelineWeeks}-week timeline</p>
              <p className="mt-2 text-sm text-(--color-muted)">{caseStudy.result}</p>
              <p className="mt-2 text-sm text-(--color-muted)">{caseStudy.scheduleFit}</p>
