  ...
  if (cohortChallenges.length === 0) {
    return (
      <div className="pb-8 sm:pb-10">
        <Card>
          <p className="font-semibold text-[var(--color-ink)]">Community board is syncing.</p>
        </Card>
      </div>
    );
  }
    ...
    <div className="pb-8 sm:pb-10">
      ...
      <Reveal className="mt-8" delayMs={60}>
        <Card className="hover-lift">
          <h2 className="text-xl font-semibold text-[var(--color-ink)]">Challenge board</h2>
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {cohortChallenges.map((challenge) => (
              <div key={challenge.title} className="surface-soft">
                <p className="font-semibold text-[var(--color-ink)]">{challenge.title}</p>
                <p className="mt-1 text-sm text-[var(--color-muted)]">{challenge.detail}</p>
                <p className="mt-2 text-xs uppercase tracking-[0.14em] text-[var(--color-brand-strong)]">
      ...
      <Reveal className="mt-4" delayMs={120}>
        <Card className="hover-lift">
          <h2 className="text-xl font-semibold text-[var(--color-ink)]">Buddy mode</h2>
