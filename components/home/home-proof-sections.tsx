  ...
  return (
    <>
          ...
          <div className="mt-6 grid gap-3">
            {faqs.map((faq) => (
              ...
              >
                ...
                <p className="mt-3 text-sm leading-7 text-(--color-muted)">{faq.answer}</p>
              </details>
            ))}
          </div>
        </section>
      ...
      <Reveal className="mt-12 sm:mt-14" delayMs={240}>
        <section className="fade-in-delayed rounded-[1.75rem] bg-foreground px-5 py-8 text-(--color-on-brand) sm:rounded-[2rem] sm:px-10 sm:py-10">
          <h2 className="font-display text-[clamp(1.7rem,5vw,2.3rem)]">
            Ready for progress you can sustain?
          </h2>
          <p className="mt-3 max-w-xl text-sm text-(--color-on-brand)/80">
            Start with the free plan today. Upgrade only when you need more structure.
          </p>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <Link
              href="/signup"
              className="inline-flex w-full items-center justify-center rounded-full bg-(--color-brand) px-5 py-3 text-base font-semibold text-(--color-on-brand) sm:w-auto"
            >
              Start Free
            </Link>
            <Link
              href="/workouts"
              className="inline-flex w-full items-center justify-center rounded-full border border-(--color-bg-soft)/30 bg-(--color-bg-soft)/80 px-5 py-3 text-base font-semibold text-(--color-on-brand) sm:w-auto"
            >
              Explore Workouts
            </Link>
          </div>
          <p className="mt-6 text-xs text-(--color-on-brand)/80">
            Have questions?{" "}
